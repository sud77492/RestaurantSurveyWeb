  ...
  $query = mysqli_query($con, "SELECT *, SUM(rspn_answer_rating), count(rspn_answer_rating), SUM(rspn_answer_rating)/count(rspn_answer_rating) as average_rating, CASE
		...
		THEN  'Very Poor'
		ELSE NULL END AS  'rating_status' FROM `tbl_surveys` LEFT JOIN `tbl_responses` ON srvy_id = rspn_survey_id WHERE rspn_user_id = '".$_SESSION['user_id']."' AND srvy_created_at >= DATE(NOW()) - INTERVAL 7 DAY GROUP BY rspn_survey_id ORDER BY srvy_id DESC  LIMIT 10 ");
    
    $query_last_week = mysqli_query($con, "SELECT count(*) as last_week_count, (SELECT count(*) FROM `tbl_surveys` WHERE srvy_created_at >= DATE(NOW()) - INTERVAL 7 DAY AND srvy_user_id = '".$_SESSION['user_id']."') as current_week_count, (SELECT count(*) as total FROM tbl_surveys WHERE datediff(current_date,date(srvy_created_at)) BETWEEN  31 AND 60 AND srvy_user_id = '".$_SESSION['user_id']."') as last_month_count, (SELECT count(*) FROM `tbl_surveys` WHERE srvy_created_at >= DATE(NOW()) - INTERVAL 30 DAY AND srvy_user_id = '".$_SESSION['user_id']."') as monthly_record FROM tbl_surveys WHERE srvy_created_at >= curdate() - INTERVAL DAYOFWEEK(curdate())+6 DAY
      AND srvy_created_at < curdate() - INTERVAL DAYOFWEEK(curdate())-1 DAY AND srvy_user_id = '".$_SESSION['user_id']."'");
    $row = mysqli_fetch_array($query_last_week);
    $count = $row['current_week_count'] / $row['last_week_count'] * 100 - 100;
