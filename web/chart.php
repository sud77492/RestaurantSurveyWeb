  ...
  //SELECT SUM(rspn_answer_rating), count(rspn_answer_rating), ROUND(SUM(rspn_answer_rating)/count(rspn_answer_rating), 2) as average_rating, (ROUND(SUM(rspn_answer_rating)/count(rspn_answer_rating), 2) / (SELECT SUM(rspn_answer_rating) FROM tbl_responses WHERE rspn_ques_id = 8 AND rspn_user_id = 4)) as average, opt_name_eng, tbl_responses.* FROM `tbl_options` INNER JOIN tbl_responses ON opt_id = rspn_answer  WHERE opt_ques_id = 8 AND rspn_user_id = 4 GROUP BY rspn_answer
  $sum = 0;
  $chartPie = mysqli_query($con, "SELECT SUM(rspn_answer_rating), count(rspn_answer_rating), ROUND(SUM(rspn_answer_rating)/count(rspn_answer_rating), 2) as average_rating, ROUND(SUM(rspn_answer_rating) / (SELECT SUM(rspn_answer_rating) FROM tbl_responses WHERE rspn_ques_id = 8 AND rspn_user_id = '".$_SESSION['user_id']."') * 100, 2) as average, opt_name_eng, tbl_responses.* FROM `tbl_options` INNER JOIN tbl_responses ON opt_id = rspn_answer  WHERE opt_ques_id = 8 AND rspn_user_id = '".$_SESSION['user_id']."' GROUP BY rspn_answer");


  while($chartPieRow = mysqli_fetch_array($chartPie)){
    $average_rating_pie[] = $chartPieRow['average'] ? $chartPieRow['average'] : 0;
    $sum = $sum + $chartPieRow['average'];
    $label[] = $chartPieRow['opt_name_eng'];
  }
  $label = "'" . implode ( "', '", $label ) . "'";
  $average_rating_pie = implode (", ", $average_rating_pie );


  $chartIndividual = mysqli_query($con,"SELECT rating_type, SUM(rspn_answer_rating), count(rspn_answer_rating), ROUND(SUM(rspn_answer_rating)/count(rspn_answer_rating), 2) as average_rating FROM `tbl_responses` INNER JOIN `tbl_questions` ON rspn_ques_id = ques_id WHERE rspn_user_id = '".$_SESSION['user_id']."' AND rating_type IS NOT NULL GROUP BY rspn_ques_id");
  while($chartData = mysqli_fetch_array($chartIndividual)){
    $average_rating[] = $chartData['average_rating'];
    $rating_type[] = substr($chartData['rating_type'], 0, 12);
  }
  $rating_type = "'" . implode ( "', '", $rating_type ) . "'";
  $average_rating = implode (", ", $average_rating );
  //echo $average_rating;exit;
  $query = mysqli_query($con, "select DATE_FORMAT(t1.srvy_created_at, '%d-%m') as survey_date, t2.srvy_user_id,
        ...
        (
          ...
          FROM tbl_surveys
          WHERE DATE_SUB(srvy_created_at, INTERVAL 1 DAY) > DATE_SUB(DATE(NOW()), INTERVAL 15 DAY) AND srvy_user_id = '".$_SESSION['user_id']."'
  ...
  while($data = mysqli_fetch_array($query)){
    $survey_per_day[] = $data['number_of_surveys'];
    $individual_date[] = $data['survey_date'];
  }
  $individual_date = "'" . implode ( "', '", $individual_date ) . "'";
        ...
        <div class="content-wrapper">
          ...
          <div class="row">
            <div class="col-md-6 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Where did you hear us?</h4>
  ...
  $(function() {
    ...
    if ($("#traffic-chart").length) {
      ...
      var trafficChartData = {
        datasets: [{
          data: <?php echo '['.$average_rating_pie.']'?>,
          backgroundColor: [
            gradientStrokeBlue,
            gradientStrokeGreen,
            gradientStrokeRed
